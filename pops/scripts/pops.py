...
def get_pops_args(argv=None):
    ...
    parser.set_defaults(use_magma_error_cov=False)
    ...
    parser.set_defaults(save_matrix_files=True)
...
def regularize_error_cov(error_cov, Y, Y_ids, gene_annot_df):
    print(gene_annot_df.head())
    #genes_annot = set(gene_annot_df.index)
    #inter = genes_annot.intersection(set(Y_ids))
    #Y_chr = gene_annot_df.loc[inter].CHR.values
...
def pops_predict(mat, rows, cols, coefs_df):
    print(coefs_df.shape)
    subset = coefs_df.loc[cols].drop_duplicates(keep="first")
    print(subset.shape)
    print(subset.head(15))
    pred = mat.dot(subset.beta.values)
